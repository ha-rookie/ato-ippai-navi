...
def main() -> None:
    ...
    for code, (name, node_id) in STATIONS.items():
        ...
        for day_type in ("weekday", "saturday_holiday"):
            path = Path(f"/tmp/meitetsu-tokoname-{code}-{day_type}.html")
    ...
    Path("/tmp/meitetsu-tokoname-last-trains.json").write_text(
        json.dumps(output, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )
    Path("/tmp/meitetsu-tokoname-route-diagnostics.json").write_text(
        json.dumps(diagnostics, ensure_ascii=False, indent=2) + "\n",
        encoding="utf-8",
    )

    print("Meitetsu Tokoname TA01-TA05 destination-specific PoC: OK")
